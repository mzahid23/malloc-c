...
struct _block *findFreeBlock(struct _block **last, size_t size) 
{
   ...
   /* Worst fit: find the largest free block that fits */
   while (curr) 
   {
      if (curr->free && curr->size >= size) 
      {
   ...
   /* Next fit: start from the last allocated block and loop through */
   ...
   if (!curr) 
   {
      /* Restart from the beginning if not found */
      curr = heapList;
      while (curr && !(curr->free && curr->size >= size)) 
      {
         *last = curr;
...
void *malloc(size_t size) 
{
   ...
   /* If the block found by findFreeBlock is larger than needed, split if space allows */
